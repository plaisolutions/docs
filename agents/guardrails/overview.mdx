...
2. **INPUT Guardrails** evaluate it
   - **Pass** — input goes to the AI Model unchanged
   - **Mask** — sensitive parts become a Masked Input, which then goes to the AI Model
   - **Block** — the interaction stops with a Blocked Message
3. **AI Model** generates an AI Response
4. **OUTPUT Guardrails** evaluate the response
   - **Pass** — the user receives the response as-is
   - **Mask** — sensitive parts become a Masked Response, which the user then receives
   - **Block** — the user receives a Safety Message instead
