...
Agent settings are organized into four tabs — **Agent**, **Model**, **Datasources**, and **Tools** — each covering a distinct part of your agent's setup, from personality and instructions to model behavior, knowledge sources, and tool access.
...
<ParamField path="multiquery_rag_enabled" type="boolean" default="false">
  Enable Multi-Query RAG to generate multiple search queries from user input
</ParamField>

**What is Multi-Query RAG?**

Instead of searching the datasource using only the user's original query, Multi-Query RAG automatically generates multiple rephrased versions of the query and searches for all variants. This improves retrieval by covering different ways of expressing the same information.

**How It Works:**

1. **User query**: "What's the password reset process?"
2. **Multi-Query RAG generates**:
...
3. **All variants are searched** — results are combined and deduplicated
4. **Agent uses the comprehensive, combined results**
...
|----------|-----------------|
| FAQ/Customer Support | ✅ Recommended - handles varied phrasing |
| Technical documentation | ✅ Recommended - people ask in different ways |
| Simple lookup (IDs, names) | ❌ Skip it - exact match queries don't need variants |
| Real-time constraints | ❌ Skip it - latency matters more than coverage |
| Large datasources | ⚠️ Use carefully - may increase costs significantly |

**Configuration Tips:**

If Multi-Query RAG is enabled, you might want to:
- Increase `vector_topk` (e.g., to 15-20) since variants may retrieve different results
- Keep `rerank_enabled` ON to filter the combined results
- Adjust `rerank_topk` down (e.g., to 3-5) to focus on best matches

**Example Setup:**

```
Vector Top K: 15 (wider net for each query variant)
Multi-Query RAG: Enabled
Enable Rerank: Yes (to filter combined results)
Rerank Top K: 5 (keep only the best)
Rerank Threshold: 0.3 (strict filtering)

Result: 3-5 highly relevant documents from comprehensive search
```

<Warning>
Multi-Query RAG increases latency and costs significantly. Use it when answer quality matters more than speed. For real-time applications, consider disabling it.
</Warning>
...
When using Google Gemini models, the interface provides **built-in tools** that are separate from your custom tools list. These are native capabilities that Google models can use automatically.

**Google Built-in Tools Available:**

Google models provide access to built-in tools such as:
...
2. Select a Google model (e.g., `gemini-2.0-flash`)
3. In the **Tools Configuration** tab, you'll see a "Google Built-in Tools" section
4. **Toggle each built-in tool** on/off independently
5. These work **alongside** your custom tools
...
**Best Practices with Google Built-in Tools:**
...
**Comparison: Google Built-in vs Custom Tools**

| Aspect | Google Built-in | Custom Tools |
|--------|-----------------|--------------|
| **Setup** | Built-in, just toggle | Requires configuration |
| **Speed** | Native to model, fast | Depends on implementation |
| **Cost** | Included in model cost | May have additional costs |
| **Flexibility** | Limited to Google offerings | Fully customizable |
| **Maintenance** | Managed by Google | You manage credentials/updates |
| **Best For** | Web search, links, native features | Custom APIs, internal systems |
...
<Tabs>
  ...
  <Tab title="Data Analyst">
    ...
    **Workflow:**
    ...
    Step 2: Agent → Calculate trends
    Step 3: Agent → Generate chart (JSON Format)
  ...
  <Tab title="Research Assistant">
    ...
    **Workflow:**
    ...
    Step 3: Agent → Extract key points
