...
Answer Filters are a powerful feature in PLai Framework that allow you to define specific response patterns for your agents. For example, you could create one so that whenever a customer asks about your refund policy, the agent always returns the same pre-approved answer instead of composing a new one from scratch each time.
...
2. **Similarity Check** — compares the query against the filter's defined trigger examples
3. **Above threshold?**
   - **Yes** — apply the filter:
     - Use Good Responses
     - Avoid Bad Responses
     - Both paths produce the **Final Response**
   - **No** — fall back to the agent's Normal Response
