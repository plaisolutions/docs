...
MCP defines a common way for any compliant server to expose its tools and resources, so you don't need a custom integration for each external service. PLai connects to these servers over HTTPS using either OAuth 2.0 or token-based authentication.
...
<ParamField path="server_url" type="input" required>
  ...
  <br />**Format**: Must be a valid URL
  <br />**Note**: HTTPS is strongly recommended for production environments
</ParamField>
...
<ParamField path="token" type="password">
...
<ParamField path="token_prefix" type="input">
  ...
  <br />**Default**: (empty string - no prefix)
  <br />**Condition**: Only shown when `auth_type` is set to `token`
  <br />**Result**: If provided, header will be formatted as `{prefix} {token}`, otherwise just `{token}`
</ParamField>
...
<Tabs>
  ...
  <Tab title="Features">
    ...
    - **Expiration Handling**: Tokens are checked for expiration (manual renewal required)
...
<Tabs>
  ...
  <Tab title="Header Formats">
    ...
    **Custom Header** (no prefix, default):
    ```
    X-API-Token: mcp_token_abc123xyz789
    ```
    
    **API Key Style** (with custom prefix):
    ```
...
- **MCP Server** exposes:
  ...
  - **Tools** — Calculators, Processors, Analyzers
  - **Prompts** — Templates, Instructions, Examples
...
- **HTTPS Recommended**: Always use HTTPS for MCP server connections in production
